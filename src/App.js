...
// Better Business Bureau accreditation for B2B — links to the business's BBB profile.
function BBBBadge({ bbb }) {
  if (!bbb) return null;
  return (
    <a href={bbb.url || "https://www.bbb.org"} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} title="Better Business Bureau profile"
      style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 8px", borderRadius: "var(--rp)", fontSize: 10, fontWeight: 700, background: "rgba(74,144,217,0.14)", color: "var(--bl)", textDecoration: "none", border: "1px solid rgba(74,144,217,0.3)" }}>
      🏛️ BBB {bbb.grade || "listed"}{bbb.accredited ? " · Accredited" : ""}
    </a>
  );
}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
              ...
              {l.b2b && l.bbb && <BBBBadge bbb={l.bbb} />}
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 116px)" }}>
      ...
      {(t.flagged || t.status === "disputed")
        ? <div style={{ padding: "11px 14px", borderTop: "1px solid var(--bd)", background: "rgba(239,93,71,0.06)" }}>
            ...
            {t.b2b && (
              <a href={`https://www.bbb.org/search?find_text=${encodeURIComponent(t.wu || "")}`} target="_blank" rel="noreferrer" style={{ display: "inline-block", marginTop: 8, fontSize: 11.5, color: "var(--bl)", fontWeight: 700, textDecoration: "none" }}>🏛️ B2B deal — also file a direct report with the Better Business Bureau →</a>
            )}
          </div>
